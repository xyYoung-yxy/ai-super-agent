  ...
  <div class="chat-container manus-app">
    ...
    <div ref="chatMessagesRef" class="chat-messages">
      <div v-for="(message, index) in messages" :key="index" :class="['message-wrapper', message.role]">
          <div v-if="message.role === 'ai'" class="avatar ai-manus">
            🧠
          </div>
...
const sendMessage = async () => {
  ...
  eventSource = new EventSource(`/api/ai/manus/chat?message=${encodeURIComponent(message)}&requestId=${chatId.value}`)
...
onMounted(() => {
  ...
  messages.value.push({
    role: 'ai',
    content: '你好！我是AI超级智能体，有什么可以帮助你的吗？',
    time: new Date().toLocaleTimeString()
  })
})
